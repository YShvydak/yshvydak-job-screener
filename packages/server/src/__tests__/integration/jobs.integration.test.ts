...
describe('Jobs API Integration', () => {
  ...
  describe('GET /api/jobs', () => {
    ...
    it('should return jobs with analysis when includeAnalysis=true', async () => {
      ...
      expect(response.body.data.jobs[0].analysis.match_score).toBe(fixtures.analysis.match_score)
    })

    it('should filter jobs by status', async () => {
      // Arrange
      seedProfile(server.db)
      seedJob(server.db, { ...fixtures.job, id: 'job-1', serpapi_job_id: 'serp_1', status: 'new' })
      seedJob(server.db, {
        ...fixtures.job,
        id: 'job-2',
    ...
    it('should filter jobs by profileId', async () => {
      ...
      // Act
      const response = await request(server.app).get('/api/jobs?profileId=profile-a').expect(200)
  ...
  describe('GET /api/jobs/:id', () => {
    it('should return job by id', async () => {
      ...
      // Act
      const response = await request(server.app).get(`/api/jobs/${fixtures.job.id}`).expect(200)
  ...
  describe('DELETE /api/jobs/:id', () => {
    it('should delete job', async () => {
      ...
      // Act
      const response = await request(server.app).delete(`/api/jobs/${fixtures.job.id}`).expect(200)
    ...
    it('should return 500 when job not found (error thrown by service)', async () => {
      // Note: Current implementation throws error which becomes 500
      const response = await request(server.app).delete('/api/jobs/non-existent-id').expect(500)
  ...
  describe('GET /api/jobs/stats', () => {
    it('should return job counts by status', async () => {
      ...
      seedProfile(server.db)
      seedJob(server.db, { ...fixtures.job, id: 'job-1', serpapi_job_id: 'serp_1', status: 'new' })
      seedJob(server.db, { ...fixtures.job, id: 'job-2', serpapi_job_id: 'serp_2', status: 'new' })
      seedJob(server.db, {
        ...fixtures.job,
        id: 'job-3',
